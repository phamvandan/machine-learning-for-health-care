...
Output:
    Calorie perday Needs to lose weight (cal): float
    Recomend: str
"""
...
# giam can trong 1 khoang thoi gian
def lose_weight(age, sex, weight, activity, height, goal_weight, days):
    if goal_weight >= weight:
        return {"Message": "Invalid input"}

    hbe = Harris_Benedict_Equation(age, sex, weight, activity)
    total_calo_subtract = (weight - goal_weight)*7716
    return {
        "Cal/day": hbe - total_calo_subtract/days,
        "Days": days
    }

# recomend thoi gian va calo moi ngay de giam can   
def recomend_lose_weight(age, sex, weight, activity, height, goal_weight):
    if goal_weight >= weight:
        return {"Message": "Invalid input"}
    ...
    return {
        "Cal/day": hbe*0.75,
        "Days": total_calo_subtract/(hbe*0.25)
    }

# API 1: giam can trong 1 khoang thoi gian
cal_per_day = lose_weight(25, "male", 60, "light", 170, 50, 60)
print(cal_per_day)
# API 2: recomend thoi gian va calo moi ngay de giam can 
print(recomend_lose_weight(25, "male", 60, "light", 170, 50))
